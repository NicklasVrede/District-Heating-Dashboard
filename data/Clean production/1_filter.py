"""
Filters production data not suitable for merging by fv_net_navn.
Loads from production_no_merge.csv, saves to:
- production_filtered.csv (plants in excluded networks)
- production_remaining.csv (plants in other networks)
"""
...
# Split data into filtered and remaining
mask = production_df['fv_net_navn'].isin(EXCLUDE_FROM_NET_MERGE)
to_filter = production_df[mask]
remaining = production_df[~mask]

# Process filtered data
filtered_df = (to_filter
...
# Save both filtered and remaining data
filtered_df.to_csv('data/Clean production/production_filtered.csv', index=False)
remaining.to_csv('data/Clean production/production_remaining.csv', index=False)
